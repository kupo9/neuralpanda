...
class NL_Event():   
    ...
    def __init__(self, nlData, eventFile = "Events.nev"):
        ...
        self._stimSegments = self._getAllStimSegments()[0]
        self.nStimSegments = len(self._stimSegments) - 1
        
        
        print (np.unique(self.nttl))
        print("\nNL_Event creating interpolator...")
...
#    
    ...
    def _getAllStimSegments(self):
        ...
        # np.where((self.eventTs == 182263784257) | (self.eventTs == 182671037085))
        return np.where(ediff > 10000000)
...
#    #instead of hardcoded 1600 values.
#    def getStimSegmentIndexAuto(self, segment):
#        allStimSeg = np.array(self._getAllStimSegments())
#        _a = np.add(allStimSeg[:self.nStimSegments],1)
#        _b = np.add(allStimSeg[1:],0) #LOOK INTO THIS, THE STOP VALUE
#        _s = np.vstack((_a,_b))
#        #this returns an (2, ) array with start, stop of the stimsegment     
...
#        arr = np.insert(arr, 1, 0)
#        arr = np.append(arr, len(self.eventTs)-1)
#        print arr
